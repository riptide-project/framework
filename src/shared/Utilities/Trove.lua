...
function Trove.BindToRenderStep(self: TroveInternal, name: string, priority: number, fn: (dt: number) -> ())
	...
	if not RunService then
		error("[Trove] BindToRenderStep is only supported in a Roblox environment", 2)
	end
...
function Trove.AttachToInstance(self: TroveInternal, instance: Instance)
	...
	elseif typeof(game) == "nil" then
		error("[Trove] AttachToInstance is only supported in a Roblox environment", 2)
