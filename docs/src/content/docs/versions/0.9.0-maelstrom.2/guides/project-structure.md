---
slug: versions/0.9.0-maelstrom.2/guides/project-structure
...
│
├── roblox_packages/            ← managed by pesde (don't edit manually)
│   └── Riptide.luau
├── pesde.toml                   ← package manifest; see Getting Started
├── pesde.lock                   ← resolved package versions
│
├── src/
...
```
ReplicatedStorage/
├── Packages/               ← roblox_packages (Riptide lives here)
...
:::tip
`SharedModulesFolder` and `ComponentsFolder` are both optional. Omit them if you don't use shared modules or CollectionService components yet. If you include a folder in the config, create and map it in your Rojo project first.
:::
