---
slug: versions/0.9.0-maelstrom.2/guides/getting-started
...
[Pesde](https://docs.pesde.dev/installation/) is the recommended package manager. In your game folder, run:

```bash
pesde init
pesde add riptide/core@0.9.0-maelstrom.2 --alias Riptide
pesde install
```

In `pesde init`, choose `roblox` and `pesde/scripts_rojo`. Result: `pesde.toml` lists Riptide, and the package is under `roblox_packages/`. Map that folder to `ReplicatedStorage.Packages` in Rojo.

### Via Wally

Run `wally init` in your game folder. Add this line under `[dependencies]` in the generated `wally.toml`:

```toml
Riptide = "riptide/core@0.9.0-maelstrom.2"
```

Run `wally install`. Result: the package is under `Packages/`. Change the `Packages` path in the Rojo project below to `"Packages"`.

### Manual Installation (.rbxm)

Download `Riptide.rbxm` from the [Maelstrom-2 release](https://github.com/riptide-project/framework/releases/tag/v0.9.0-maelstrom.2). For this option, remove the `Packages` mapping from the Rojo project below, then insert the model in Roblox Studio as `ReplicatedStorage/Packages/Riptide`.
