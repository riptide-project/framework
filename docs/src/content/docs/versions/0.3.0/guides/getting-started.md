---
slug: versions/0.3.0/guides/getting-started
title: Getting started with 0.3.0
description: Install and launch Riptide 0.3.0.
---
...
The Wally registry lists 0.3.0 under `thereplicatedfirst/riptide`, even though the tag’s `wally.toml` and README use `riptide-project/riptide`. Use the published registry name below, then run `wally install`. If this is a new project, run `wally init` first and add the dependency to the generated `wally.toml`:
...
Alternatively, download `Riptide.rbxm` from the [v0.3.0 release](https://github.com/riptide-project/framework/releases/tag/v0.3.0) and insert it as `ReplicatedStorage/Packages/Riptide` in Studio.
