---
slug: versions/0.9.0-maelstrom.3/guides/plugins
title: Plugins
...
Normal plugins do not receive raw core event access such as `OnCoreEvent`. Use namespaced sandbox events instead. Trusted plugin permissions may be added later, but they are intentionally not part of the normal Maelstrom-3 sandbox.
...
Missing required dependencies are strict in Maelstrom-3: the plugin is not loaded, and dependents of that blocked plugin are blocked as well. Optional dependencies are not yet a separate manifest field.
