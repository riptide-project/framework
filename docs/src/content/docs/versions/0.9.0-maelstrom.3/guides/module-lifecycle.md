---
slug: versions/0.9.0-maelstrom.3/guides/module-lifecycle
...
Player initialization runs independently for each player. A yielding `OnPlayerAdded` hook for one player does not hold up initialization for another player.
...
function DataService:Init(Riptide)
    ...
    Riptide.Network.Register("GetCoins", function(player)
        Riptide.Network.FireClient(player, "CoinsResponse", Riptide.State:Get("coins", player))
    end)
