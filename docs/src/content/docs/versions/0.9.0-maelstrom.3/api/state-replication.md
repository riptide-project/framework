---
slug: versions/0.9.0-maelstrom.3/api/state-replication
...
Typed key proxies introduced in Maelstrom-2 remain available. Dynamic string methods remain available, but typed proxies give autocomplete for state keys and typed values.
...
The request is **asynchronous**. It includes a request ID and has a deadline with at most three attempts. Live deltas keep applying while the snapshot is in flight. When the snapshot arrives, per-key versions preserve values from newer deltas, including deletions.
...
```
Client connects
  → FireServer("__riptide_state_snapshot", requestId)     [request]
...
If the request arrives during the per-player cooldown, the server may send:
  → FireClient(player, "__riptide_state_snapshot", {
      requestId = requestId,
      retryAfter = seconds,
    })
...
- Deltas remain live during snapshot sync. Per-key versions prevent an older snapshot from rolling them back.
- Internal state traffic uses the reliable remote and is separate from public Network handlers and middleware.
