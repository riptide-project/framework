---
slug: versions/0.9.0-maelstrom.3/api/network
title: Network
description: Unified event communication between client and server.
---


The `Network` module provides a unified communication layer over a `RemoteEvent` / `UnreliableRemoteEvent` pair. Game networking uses named string events without manual remote management.

Names beginning with `__riptide_` are reserved for framework messages. Public `Register`, `Unregister` and fire methods reject them. Internal state messages share the reliable remote, but use a separate handler route: public middleware does not see or change them, and they are ignored on the unreliable remote.

> **Maelstrom breaking change:** `RemoteFunction`-based APIs (`InvokeClient`, `InvokeServer`) have been removed. Use paired `FireServer`/`FireClient` events with a request ID for request/response patterns; add an application timeout when a reply is required.
...
Typed proxy factories introduced in Maelstrom-2 remain available. They keep the dynamic string APIs available, but let Luau autocomplete event names as fields and check payloads from your event map.
...
For request/response patterns, use a pair of events. Register the response handler before sending the request, and include a request ID if several requests may overlap. `Network.Register` does not return a response or wait for one:

```lua
-- Server: respond to a client request
Riptide.Server.Network.Register("GetInventory", function(player, requestId)
    Riptide.Server.Network.FireClient(player, "GetInventory_Response", requestId, getPlayerInventory(player))
end)

-- Client: register first, then send
local requestId = 1
local function onInventory(responseId, data)
    if responseId ~= requestId then return end
    Riptide.Client.Network.Unregister("GetInventory_Response", onInventory)
    renderInventory(data)
end
Riptide.Client.Network.Register("GetInventory_Response", onInventory)
Riptide.Client.Network.FireServer("GetInventory", requestId)
```
