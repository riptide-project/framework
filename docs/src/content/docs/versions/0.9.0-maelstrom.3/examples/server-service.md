---
slug: versions/0.9.0-maelstrom.3/examples/server-service
title: Server Service
description: A small server service using the injected server API, typed validation, and player state.
---

This example shows the recommended Maelstrom-3 shape for a server module. Server modules receive the side-specific server API in lifecycle hooks, so `Riptide.Network`, `Riptide.State`, and `Riptide.GetService` autocomplete server-only methods.
