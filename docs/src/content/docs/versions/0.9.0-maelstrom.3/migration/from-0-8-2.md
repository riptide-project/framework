---
slug: versions/0.9.0-maelstrom.3/migration/from-0-8-2
title: From 0.8.2 Stable
description: Migration checklist for moving from the 0.8.2 stable release to 0.9.0-maelstrom.3.
---

Maelstrom-3 is a preview release. It includes breaking changes that improve type quality, plugin extensibility, network safety, and runtime determinism.

Use this page when migrating from the `0.8.2` stable release.

## Package Version

Update the package version to `0.9.0-maelstrom.3`. The [getting-started guide](../../guides/getting-started/) uses `pesde init` and `pesde add` for a new project.
...
The important change is the value passed into the module: server modules receive `Riptide.Server`, and client modules receive `Riptide.Client`. Inside a server service, register events and access state through that injected API:
...
```lua
-- Server middleware
Riptide.Network.UseMiddleware("server", function(player, eventName, args)
...
State remains server-authoritative, but Maelstrom-3 hardens malformed payload handling and adds typed key proxies.
...
- Start the game in Studio and check server/client startup and the migrated features.
- If you are already on Maelstrom-2, review the shorter [Maelstrom-2 to Maelstrom-3 guide](../from-maelstrom-2/).
