---
slug: versions/0.7.0/api/component-service
title: Tagged components
description: Build CollectionService components in Riptide 0.7.0.
---
...
Look up a component with `Riptide.ComponentService:Get(instance, "Sparkle")`. With several tags on one instance, `Get(instance)` returns `nil` and warns. Pass the tag name to select one component.
